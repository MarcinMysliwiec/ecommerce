...
const ProductScreen = ({ match, history }) => {
  ...
  const qtyOptions = product ? [...Array(product.numInStock).keys()].map((p) => ({
    text: `${p + 1}`,
    value: `${p + 1}`
  })) : [];
          ...
          <div className="product">
            <div className="product__info">
              ...
              <div className="product__detail">
                <p>{product.detail}</p>
                <div>{product.numInStock > 0 ? <span style={{ color: 'green' }}>In Stock.</span> : <span
                  style={{ color: '#f68872' }}>Out Of Stock.</span>}</div>
              </div>

              <Form className="product__form">
                <Form.Field control={Select} onChange={getQty} options={qtyOptions} defaultValue="1"
                            disabled={product.numInStock === 0}/>
                <Button color="black" type="button" disabled={product.numInStock === 0}
