...
const AdminProductEditScreen = ({ match, history }) => {
  ...
  const { success: successUpdate } = adminUpdateProduct;
      ...
      {loadingProduct ? (
        ...
        <FormContainer>
          ...
          <Form onSubmit={(e) => submitHandler(e)} loading={loadingProduct}>
            <Form.Input size="large" icon="shopping basket" iconPosition="left" label="Name" placeholder="Name"
                        onChange={(e) => setName(e.target.value)} value={name}/>
            <Form.TextArea size="large" label="Datail" placeholder="Detail" onChange={(e) => setDetail(e.target.value)}
                           value={detail} style={{ height: '150px' }}/>

            <Form.Input size="large" icon="dollar sign" iconPosition="left" label="Price" type="text"
                        placeholder="Price" onChange={(e) => setPrice(e.target.value)} value={price}/>
            <Form.Input size="large" icon="hashtag" iconPosition="left" label="NumInStock" type="text"
                        placeholder="NumInStock" onChange={(e) => setNumInStock(e.target.value)} value={numInStock}/>

            <Form.Input size="large" icon="image outline" iconPosition="left" label="Image" type="text" value={image}/>

            <div {...getRootProps({ className: 'dropzone' })}>
              <input {...getInputProps()} />
              {isDragActive ? <p>Drop the files here ...</p> : <p>Drag 'n' drop image file here, or click to select
                files. (Only *.jpeg and *.png)</p>}
            </div>

            {acceptedFiles.length ? <Message positive>Accepted. {file}</Message> : null}
            {fileRejections.length ? <Message negative>This image format is not supported. Only *.jpeg and *.png images
              will be accepted</Message> : null}
