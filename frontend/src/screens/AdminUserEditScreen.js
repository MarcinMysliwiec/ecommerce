...
import { Button, Checkbox, Form, Label, Message, Popup } from 'semantic-ui-react';
...
const AdminUserEditScreen = ({ match, history }) => {
  ...
  return (<>
    ...
    {loading ? (<Loader/>) : error ? (<Message variant="danger">{error}</Message>) : (<FormContainer>
      ...
      <Form onSubmit={(e) => submitHandler(e)} loading={loading}>
        ...
        <Form.Field>
          <label>
            Admin Status:
            {user.isAdmin ? (<span style={{ marginLeft: '5px', fontWeight: '100', color: 'green' }}> Admin</span>) : (
          ...
          <Popup
            ...
            />}
            content={<Checkbox slider label={isAdmin ? 'Remove' : 'Add'}
                               onChange={() => setIsAdmin(!isAdmin)} checked={isAdmin}/>}
            on="click"
            position="top right"
          />
        </Form.Field>
